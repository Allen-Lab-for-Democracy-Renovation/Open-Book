...
export default function UploadPage() {
  ...
  const [replaceConfirmation, setReplaceConfirmation] = useState<{
    rowCount: number;
    fiscalYears: string[];
    sources: { uploadId: string; fileName: string }[];
  } | null>(null);
  ...
  const handleConfirmMapping = async (confirmReplace = false) => {
    ...
    try {
      ...
      if (res.status === 409 && data.requiresConfirmation) {
        setReplaceConfirmation(data.affected);
        return;
      }
        ...
        <div className="space-y-6">
          ...
          {replaceConfirmation && (
            ...
            >
              ...
              <p className="text-sm text-amber-700 mb-3">
                {category ? CATEGORY_LABELS[category] : "This category"}{" "}
                already has {replaceConfirmation.rowCount.toLocaleString()}{" "}
                row{replaceConfirmation.rowCount === 1 ? "" : "s"} for fiscal
                year{replaceConfirmation.fiscalYears.length === 1 ? "" : "s"}{" "}
                {replaceConfirmation.fiscalYears.join(", ")} from a different
                upload
                {replaceConfirmation.sources.length > 0 && (
                  <>
                    {" "}
                    (
                    {replaceConfirmation.sources
                      .map((s) => s.fileName)
                      .join(", ")}
                    )
                  </>
                )}
                . Saving this file will permanently delete those existing
                rows and replace them with the data in this upload. This
                cannot be undone.
              </p>
              <div className="flex items-center gap-3">
                <button
                  ...
                  disabled={saving}
                  className="px-4 py-2 bg-amber-700 text-white rounded-md text-sm font-medium hover:bg-amber-800 disabled:opacity-50 transition-colors"
                >
                  {saving ? "Replacing..." : "Replace existing data"}
                </button>
                <button
